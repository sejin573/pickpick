...
- **테스트 계정:** 게스트 추천은 계정 없이 사용 가능 / 대화 저장은 이메일·비밀번호 로그인 사용
- **현재 버전:** v1.9.0
...
| 테스트 계정 | 게스트 추천은 별도 계정 없이 사용할 수 있으며, 대화 저장 기능은 사용자의 이메일·비밀번호 로그인을 사용합니다. |
...
로그인 사용자가 이전 대화를 다시 열었을 때 최신 상품 검색을 재실행하지 않고, 추천을 받았던 당시의 결과를 그대로 확인할 수 있도록 대화 저장 방식을 강화했습니다.

- 상품명, 가격, 이미지 URL, 구매 링크, 판매처, 추천 점수와 추천 근거를 포함한 전체 응답 저장
- `snapshotVersion`, `savedAt`, `response`로 구성된 버전형 JSON 스냅샷 적용
- 과거에 저장된 기존 payload 형식도 자동으로 판별하여 계속 복원
- 저장된 대화 화면에 “저장된 당시 상품 정보”와 저장 시각 표시
- 외부 상품 가격이나 검색 결과가 변경되어도 기존 대화에서는 저장 당시 결과 유지
- 기존 `messages.payload` JSONB 컬럼을 활용하므로 별도 Supabase 마이그레이션 불필요
- 비로그인 사용자는 기존처럼 일회성 추천으로 이용

## v1.7.0 업데이트

- 한 대화 안에서 “다른 거 추천해줘”, “20만원대로 보여줘”처럼 후속 요청 가능
- 이전 요청의 대상·상황·예산 맥락을 유지하면서 이미 보여준 상품은 다음 추천에서 제외
- 로그인 사용자의 여러 질문과 추천 스냅샷을 하나의 대화에 순서대로 누적 저장
- 과거 대화를 열면 해당 대화의 모든 질문과 당시 추천 상품을 그대로 복원
- 혈압계·혈압측정기를 검색 계획, 실시간 상품, 샘플 데이터에서 모두 제외
- 답변 카드가 순서대로 등장할 때 화면이 자연스럽게 아래로 따라가는 자동 스크롤 적용
- 데스크톱 하단 채팅 입력창이 사이드바 아래로 잘리던 문제 수정
- 입력창과 마지막 상품 카드가 겹치지 않도록 하단 여백과 안전 영역 보강

## v1.8.0 업데이트

- 이메일을 아이디로 사용하는 이메일·비밀번호 로그인 적용
- 신규 가입자는 최초 회원가입 때만 이메일 인증 진행
- 인증 완료 후에는 이메일함을 다시 확인하지 않고 비밀번호로 바로 로그인
- 회원가입, 로그인, 비밀번호 재설정과 새 비밀번호 등록을 하나의 인증 화면에서 지원
- 기존 Magic Link 사용자는 비밀번호 재설정 메일로 비밀번호를 등록한 뒤 기존 대화 그대로 이용 가능
- 답변 순차 모션의 자동 스크롤은 추천 상품 카드가 화면 중앙에 보이는 지점에서 정지
- 비교표·구매 가이드·페이지 맨 아래로 강제 이동하지 않도록 스크롤 동작 수정

## v1.8.1 업데이트

- 첫 요청과 후속 채팅 응답 모두 추천 상품 카드의 시작 지점으로 자동 스크롤
- 추천 제목, 가격대·카테고리 탭과 첫 상품 이미지가 함께 보이는 위치로 고정
- 이전 대화 컨테이너나 페이지 맨 아래로 이동하던 불필요한 스크롤 제거
- 메시지를 전송한 즉시 하단 채팅 입력창을 비우고 응답 후에도 빈 상태 유지

## v1.8.2 업데이트

- 질문 전송 직후 새 사용자 채팅 말풍선이 보이는 위치로 1차 자동 스크롤
- 추천 응답이 준비되면 상품 카드 시작 지점으로 2차 자동 스크롤
- 첫 요청과 “다른 상품 찾아줘” 같은 후속 요청에 동일한 2단계 흐름 적용

## v1.9.0 업데이트

- 추천 파이프라인을 단순 함수 호출이 아니라 `goal → state → action → observation → trace`를 남기는 PickPick Agent Runtime으로 확장
- 사용자별 학습 메모리(`user_preference_profiles`)를 추가하여 로그인 사용자의 선호 카테고리, 선호/비선호 키워드, 최근 가격대와 관찰 기록을 다음 추천에 반영
- 추천 실행마다 `agent_learning_events`에 입력, 분석 결과, 실행 trace, 제외 상품 수, 추천 상품 ID를 저장하여 이후 품질 개선 데이터로 활용 가능
- 혈압계, 혈당계, 의료기기, 영양제, 중고, 리퍼, 부품, 케이스 등 부적합 상품을 한 번 더 검토하는 catalog review tool 추가
- `/api/agent/feedback` API를 추가하여 추후 좋아요/별로예요/비슷한 상품 요청 버튼을 학습 데이터로 누적할 수 있는 기반 마련
- Supabase CLI를 devDependency로 추가하고 원격 DB에 agent learning migration 적용 완료

## 서비스 화면

### 짧은 시연 영상

로그인·회원가입·비밀번호 재설정, 자연어 추천 요청, 가격대·카테고리 전환, 상품 비교, 구매 가이드와 후속 질문까지 전체 흐름을 약 53초의 1080p 영상으로 담았습니다.

[▶ PickPick v1.8.2 고화질 시연 영상 보기](docs/videos/pickpick-demo-v182-1080p.mp4)
...
이메일을 아이디로 사용하며 첫 회원가입 때만 인증 메일을 확인합니다. 이후에는 이메일과 비밀번호로 바로 로그인합니다.
...
- 혈압계·혈압측정기는 검색 계획, 네이버 실상품 결과와 내장 fallback 데이터에서 모두 제외
...
- 같은 채팅에서 `다른 상품 추천해줘`, `20만원대로 보여줘` 같은 후속 질문을 입력하면 최초 요청의 대상·상황·예산 맥락을 유지
- 이전 턴에서 보여준 상품 ID를 다음 추천 후보에서 제외해 새로운 선택지를 우선 제공
- 검색을 시작하면 검색바가 화면 위에서 하단으로 슬라이드 다운하며 도킹, 이후 입력창은 화면 하단에 고정
- 메시지를 전송하면 입력창을 즉시 비우고 응답이 완료된 뒤에도 빈 상태 유지
- 질문 전송 시 새 말풍선으로 먼저 이동하고, 상품 준비가 끝나면 추천 카드 시작 지점으로 다시 이동하는 2단계 자동 스크롤
- 추천 카드 이동 시 제목·가격대·카테고리 탭과 첫 상품 이미지가 보이는 위치에서 정지
- 진행 단계가 인라인 채팅 카드로 흘러나오고, 결과 도착 후에도 카드가 사라지지 않고 모든 단계가 ✓로 정리됨
- 분석 패널과 결과 카드들이 stagger 애니메이션으로 부드럽게 순차 등장
- 결과 메시지는 정해진 순서에 따라 한 장씩 자연스럽게 reveal
...
- Supabase Auth의 이메일·비밀번호 로그인과 최초 가입 이메일 인증
- 로그인 사용자의 자연어 요청과 전체 추천 응답을 PostgreSQL JSONB 스냅샷으로 자동 저장
- ChatGPT 형태의 좌측 사이드바에서 최근 대화 목록 확인
- 저장된 대화를 선택하면 추천 API를 다시 호출하지 않고 당시 상품명·가격·이미지·링크·추천 근거를 그대로 복원
- 같은 대화에서 후속 조건을 입력하면 이전 맥락을 이어받아 새로운 상품을 추가 추천
- 여러 질문과 추천 결과를 하나의 conversation에 시간순으로 누적하고 전체 턴을 함께 복원
- 스냅샷 버전과 저장 시각을 함께 기록하고, 기존 저장 형식도 하위 호환
- 과거 대화 화면에서 상품 정보가 저장된 시각을 안내
...
- 로그인 사용자의 추천 실행 기록을 `agent_learning_events`에 저장
- 사용자별 선호 프로필을 `user_preference_profiles`에 누적
- 이전 추천에서 자주 등장한 카테고리, 키워드, 가격대를 다음 검색 계획과 상품 점수에 반영
- 비선호 카테고리와 키워드는 다음 추천의 catalog review 단계에서 제외 신호로 사용
- 각 추천 응답의 `meta.agentRunId`, `meta.agentTrace`, `meta.learningMemoryApplied`로 agent 실행 과정 확인 가능
- 명시 피드백 API(`/api/agent/feedback`)를 통해 추후 상품별 좋아요/별로예요 버튼을 개인화 학습 데이터로 연결 가능
...
| Agent | PickPick Agent Runtime, action trace, learning memory, catalog review tool |
...
2. Project URL과 Publishable Key를 `.env.local`과 Vercel 환경 변수에 등록합니다.
3. Supabase Auth의 Site URL을 배포 URL로 설정합니다.
4. Redirect URLs에 로컬 및 배포 콜백을 등록합니다.
...
5. Supabase CLI 또는 SQL Editor로 migration을 적용합니다.
...
- [`supabase/migrations/202606220001_chat_history.sql`](supabase/migrations/202606220001_chat_history.sql): 대화, 메시지, RLS
- [`supabase/migrations/202607080001_agent_learning.sql`](supabase/migrations/202607080001_agent_learning.sql): agent 실행 로그, 사용자 선호 프로필, RLS

회원가입 인증 및 비밀번호 재설정 메일은 `/auth/confirm`에서 OTP token hash 또는 PKCE code를 세션으로 교환합니다. 환경 변수가 없으면 로그인 UI 대신 설정 안내가 표시되고 쇼핑 추천은 계속 사용할 수 있습니다.
...
- 게스트 추천은 별도의 테스트 계정 없이 바로 사용 가능
- 로그인 기능은 Supabase 이메일·비밀번호 방식으로 제공하며 최초 가입 때 이메일 인증 필요
- 대화 저장과 이전 추천 복원은 사용자가 본인의 이메일로 로그인한 경우 활성화
- 공용 테스트 계정이나 비밀번호는 사용하지 않음

## 9. LLM / Agent 동작 구조

1. **Agent Runtime 생성:** 요청마다 `runId`, `goal`, `state`, `trace`를 가진 PickPick Agent 실행 상태를 생성합니다.
2. **학습 메모리 로드:** 로그인 사용자의 `user_preference_profiles`에서 선호 카테고리, 선호/비선호 키워드, 최근 가격대와 관찰 기록을 읽습니다.
3. **후속 대화 문맥 결합:** 이전 질문이 있으면 최초 요청의 대상·상황·예산과 현재 조건을 결합하고 기존 추천 상품 ID를 제외합니다.
4. **사용자 의도 분석:** 선물, 구매, 휴식, 집중, 수면, 식물/플랜테리어 등 요청 목적을 파악하고 조건을 구조화합니다.
5. **OpenAI 검색 계획:** 자유 입력과 학습 메모리를 실제 구매 가능한 세 가지 상품 관점과 네이버 검색어 2~4개씩으로 정리합니다. 제공된 요청에서 벗어난 서비스나 상품군은 만들지 않도록 구조화 출력으로 제한합니다.
6. **규칙 fallback:** OpenAI 키가 없거나 계획 호출이 실패하면 내장 키워드 사전과 카테고리 규칙으로 같은 형식의 검색 계획을 생성합니다.
7. **실상품 검색:** 네이버 쇼핑 API에서 카테고리별 후보를 조회하고 각 결과의 검색 순위를 보존합니다.
8. **Agent catalog review:** 혈압계, 혈당계, 의료기기, 영양제, 중고, 리퍼, 부품, 케이스 등 부적합 상품과 사용자 비선호 신호에 맞는 상품을 제외합니다.
9. **코드 기반 1차 랭킹:** 예산, 키워드 적합도, 실용성, 감성, 가성비, 리스크에 더해 네이버 검색 순위, 메이저 쇼핑몰 입점, 브랜드 보유 여부와 누적 선호 신호를 가중합니다.
10. **예산에 따른 분기:**
...
11. **설명 생성:** 선택 근거와 적합한 사용자를 실제 후보 정보에 근거해 작성합니다.
12. **구매 가이드 생성:** 비교 결과와 구매 전 확인 사항을 제공합니다.
13. **학습 이벤트 저장:** 로그인 사용자의 agent 실행 trace, 추천 상품 ID, catalog review 결과와 요약 관찰을 `agent_learning_events`와 `user_preference_profiles`에 저장합니다.
...
사용자 자연어 입력
  → POST /api/recommend
  → [후속 질문] 이전 요청 맥락 결합 + 이미 보여준 상품 ID 제외
  → 의도·예산·대상·상황 분석
  ...
  → 질문 말풍선 위치로 1차 스크롤
  → 상품 카드가 준비되면 추천 제목·탭·첫 이미지 위치로 2차 스크롤
  → [로그인 사용자] 같은 conversation에 질문과 추천 스냅샷을 턴 단위로 누적
  → [과거 대화 열기] 추천 API 재호출 없이 저장된 스냅샷을 그대로 복원
                    · 상품명·가격·이미지·링크·추천 근거 유지
                    · 스냅샷 버전과 저장 시각 표시
                    · 여러 질문과 추천 턴을 시간순으로 전체 복원
```

### v1.9.0 Agent 학습 데이터 흐름

```text
사용자 요청
  → PickPick Agent Runtime 생성(runId, goal, state, trace)
  → 로그인 사용자라면 user_preference_profiles에서 학습 메모리 조회
  → 이전 대화와 제외 상품 ID를 반영해 요청 재구성
  → intent / search plan / product search action 실행
  → catalog review tool이 부적합 상품과 비선호 신호를 검사
  → 추천 결과 생성 후 meta.agentTrace에 action별 observation 기록
  → 로그인 사용자라면 agent_learning_events에 실행 로그 저장
  → user_preference_profiles에 선호 카테고리, 키워드, 가격대 관찰 누적
  → 다음 추천에서 누적 메모리를 검색 계획과 상품 점수에 반영
```

이 구조는 모델 파라미터를 실시간으로 업데이트하는 방식이 아니라, 서비스 내부에 쌓이는 사용자 행동·추천 결과·피드백 데이터를 agent memory로 활용하는 방식입니다. 운영 중에는 안정적으로 개인화 품질을 높이고, 이후에는 이 데이터를 평가셋, RAG 메모리, fine-tuning 후보 데이터로 확장할 수 있습니다.
...
```json
{
  "message": "다른 상품도 추천해줘",
  "context": {
    "messages": ["친구 집들이 선물 30만원대 추천해줘"],
    "excludedProductIds": ["naver-product-id-1"]
  }
}
```
...
- 상품명에 그룹별 핵심 제품 명사가 포함된 완제품만 허용하고 수량·옵션 표기 중복을 축소
- 상품이 자동 전환되는 카테고리 탭·슬라이드 UI
- ChatGPT 풍 대화형 결과 페이지: 사용자 말풍선, 모든 결과의 `PICKPICK AGENT` 메시지 래핑, 검색바 하단 슬라이드 다운 도킹
- 후속 질문 문맥 결합과 이전 상품 ID 제외를 통한 다중 턴 재추천
- 질문 말풍선 → 상품 카드 순서의 2단계 자동 스크롤과 순차 reveal 모션
- 전송 직후 입력창 초기화와 데스크톱 사이드바를 고려한 하단 입력창 배치
...
- Supabase 이메일·비밀번호 로그인, 최초 이메일 인증과 쿠키 기반 세션
...
- PickPick Agent Runtime을 이용한 action trace, 실행 상태와 관찰 결과 기록
- `agent_learning_events`, `user_preference_profiles` 기반의 로그인 사용자별 학습 메모리
- 누적 선호·비선호 카테고리와 키워드를 다음 추천의 검색 계획, 상품 검토와 점수에 반영
- `/api/agent/feedback`을 통한 상품별 명시 피드백 수집 기반 마련
...
Supabase 기반 대화 저장과 사용자별 Agent 학습 메모리는 구현했으며, 다음 단계에서는 이 데이터를 더 명시적인 사용자 행동과 연결해 추천 품질을 높일 계획입니다.

- 추천 카드에 좋아요·별로예요·비슷한 상품·덜 비슷한 상품 버튼 추가
- `/api/agent/feedback`으로 수집한 명시 피드백을 UI에서 바로 반영
- 사용자별 선호 프로필을 대시보드나 설정 화면에서 확인·초기화하는 기능
- Google·Kakao·Naver 등 SNS 계정을 이용한 소셜 로그인
- 로그인과 연동한 상품 즐겨찾기
- 대화 검색, 보관 처리와 사용자별 데이터 보존 기간 설정
- 추천 클릭·구매 전환 데이터를 활용한 평가셋 구축
- 누적된 agent learning event를 기반으로 RAG 메모리 또는 fine-tuning 후보 데이터셋 확장

현재는 로그인한 사용자의 요청, 추천 결과, agent 실행 trace와 선호 프로필을 저장하며, 비로그인 사용자의 대화는 서버에 영구 저장하지 않습니다.
...
- 실제 운영 화면 캡처
- `npm run record:demo`를 이용한 1분 이내 운영 화면 시연 영상 생성
...
app/
  ...
  api/agent/feedback/      # agent 학습 피드백 수집 API
  api/recommend/route.ts   # 후속 문맥·상품 제외·가격대 그룹을 처리하는 추천 API
  auth/confirm/route.ts    # 회원가입 인증·비밀번호 재설정 / PKCE 콜백
  globals.css              # scroll-reveal, composer-dock-in 등 트랜지션
  layout.tsx               # Pretendard 폰트 로드
  page.tsx                 # 다중 턴 채팅, 2단계 자동 스크롤, 요청 취소, 푸터 버전
components/
  AssistantMessage.tsx     # 결과 영역 공통 P 아바타 + PICKPICK AGENT 래퍼
  AuthDialog.tsx           # 이메일 회원가입·비밀번호 로그인·재설정
...
lib/
  ...
  agent/                   # PickPick Agent Runtime, learning memory, catalog review tool
...
supabase/migrations/
  ...
  202607080001_agent_learning.sql # agent_learning_events/user_preference_profiles/RLS 스키마
