...
- 조건 분석, 실상품 검색, 품질 검사, OpenAI 비교와 구매 가이드 생성 단계를 보여주는 로딩 화면
...
6. **코드 기반 1차 랭킹:** 예산, 키워드 적합도, 실용성, 감성, 가성비와 리스크를 계산합니다.
7. **OpenAI 재랭킹(선택):** 카테고리별 상위 후보 6개 중 사용자 맥락에 가장 적합한 상품 3개를 다시 선정합니다.
...
사용자 자연어 입력
  ...
  → 네이버 쇼핑 API 병렬 검색
  → 가격·상품 유형·노이즈 필터
  → 카테고리별 코드 기반 1차 점수 계산
  → 카테고리별 상위 후보 6개 압축
  → OpenAI가 최종 상품 3개 재선정 및 설명 생성(선택)
