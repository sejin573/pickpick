...
- 데이터베이스, 로그인, 즐겨찾기와 사용자별 추천 기록
- 현재 대화와 추천 결과를 새로고침 이후에도 유지하는 영구 저장
- 재고와 배송 예정일의 실시간 검증
- 결제 또는 장바구니 연동
...
다음 단계에서는 데이터베이스를 연결해 대화와 추천 결과를 저장하는 기능을 우선 구현할 계획입니다.

- 사용자별 이전 대화와 자연어 쇼핑 요청 저장
- 대화에서 생성된 분석 결과, 가격대, 추천 상품과 구매 가이드 저장
- 새로고침하거나 다시 접속해도 이전 대화를 이어서 확인하는 히스토리 기능
- 저장된 관심사와 선택 이력을 다음 추천에 반영하는 개인화
- 로그인과 연동한 즐겨찾기, 대화 삭제와 보관 기간 관리

초기에는 PostgreSQL 또는 Supabase를 연결하고, 사용자·대화·메시지·추천 결과를 분리해 저장하는 구조를 고려할 수 있습니다. 현재 버전은 별도의 데이터베이스 없이 한 번의 요청과 응답 안에서 동작하므로 개인정보를 서버에 영구 저장하지 않습니다.
